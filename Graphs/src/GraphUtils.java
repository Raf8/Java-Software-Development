...
public class GraphUtils {
	...
	public static int minDistance(Graph graph, String src, String dest) {
		...
		Node src_node = graph.getNode(src); //get the source node
		Node dest_node = graph.getNode(dest); //get the destination node
		
		if (!graph.containsNode(src_node)) {
			return -1;
		}
		if (src_node.getElement().equals(dest)) {
			return 0;
		}
		Set<Node> marked = new HashSet<Node>(); // Set for marked nodes
		Queue<Node> to_visit = new LinkedList<>(); // Queue for the nodes to visit
		Map<Node, Integer> distance = new HashMap<>();
		distance.put(src_node, 0);
		
		to_visit.add(src_node); // add the source node 
		while(!to_visit.isEmpty()){
			Node current = to_visit.remove();
			for (Node neighbor : graph.getNodeNeighbors(current)) {
				if (!marked.contains(neighbor)) {
					distance.put(neighbor, distance.get(current) + 1);
					if (neighbor.equals(dest_node)) {
